...
export default function PaymentHistory() {
  const { user } = useAuth()
  const payments = mockPayments.filter(p => p.userEmail === user?.email)
      ...
      {payments.length === 0 ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table className="data-table">
                ...
                <tbody>
                  {payments.map(p => (
                    <tr key={p.id}>
                      <td style={{ fontFamily: 'JetBrains Mono', fontSize: '0.75rem', color: '#5a5a78' }}>{p.id}</td>
                      ...
                      <td style={{ color: '#6060a0', fontFamily: 'JetBrains Mono', fontSize: '0.78rem' }}>{p.date}</td>
          ...
          <div style={{ marginTop: '1.25rem', background: '#0e0e18', border: '1px solid #1a1a28', borderRadius: '0.625rem', padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontFamily: 'JetBrains Mono', fontWeight: 700, color: '#e8e8f0', fontSize: '1.1rem' }}>${payments.reduce((s, p) => s + p.amount, 0)}</span>
          </div>
        </motion.div>
      )}
    </div>
